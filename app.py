...
from sqlalchemy import (
    create_engine, Column, String, DateTime, Integer, ForeignKey, Text, select, func
)
...
# For users: license format selector
LICENSE_PREFIX = _env("LICENSE_PREFIX", "LIC")
LICENSE_RANDOM_LEN = int(_env("LICENSE_RANDOM_LEN", "20"))  # LIC + 20 chars => easy enough
...
class License(Base):
    ...
    status = Column(String(16), default="active")  # active/revoked/disabled
...
engine = create_engine(DATABASE_URL, pool_pre_ping=True, connect_args=_connect_args_for_postgres(DATABASE_URL))
...
# =========================
# Schemas (match your tool's license_manager.py)
# =========================
...
def _startup():
    # Create tables (will wake Neon on first connect)
    Base.metadata.create_all(bind=engine)
    # Validate key early (fail-fast)
...
def public_keys():
    # IMPORTANT: matches tool's bootstrap parser (expects kid + public_key_b64 at root)
...
def _generate_license_key() -> str:
    # LIC- + 20 chars by default (A-Z0-9). User requested "LIC-16" style:
    # if you truly want 16 random chars, set LICENSE_RANDOM_LEN=16 in env.
...
def activate(req: ActivateReq, sess=Depends(db_sess)):
    ...
    # Rotate refresh token on activate (simple & safe)
...
def admin_list_licenses(q: str = "", _tok=Depends(admin_auth), sess=Depends(db_sess)):
    q = (q or "").strip()
    stmt = select(License)
    if q:
        stmt = stmt.where(License.key_tail.ilike(f"%{q}%"))
...
def admin_revoke_license(license_id: str, _tok=Depends(admin_auth), sess=Depends(db_sess)):
    ...
    acts = sess.execute(select(Activation).where(Activation.license_id == lic.id, Activation.revoked_at.is_(None))).scalars().all()
    for a in acts:
        a.revoked_at = _now()
